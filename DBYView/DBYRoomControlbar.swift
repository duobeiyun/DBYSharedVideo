...
class DBYRoomControlbar: UIView {
    ...
    func showVideoButton() {
        ...
        videoButton.setTitle(" 退出上台", for: .selected)
    ...
    @objc func cameraRequest(sender: UIButton) {
        ...
        if sender.isSelected {
            sender.layer.backgroundColor = DBYStyle.yellow.cgColor
            sender.layer.cornerRadius = 4
        }else {
            sender.layer.backgroundColor = UIColor.white.cgColor
            sender.layer.cornerRadius = 0
        }
        delegate?.roomControlBar(owner: self, didClickCameraRequest: sender.isSelected)
    }
